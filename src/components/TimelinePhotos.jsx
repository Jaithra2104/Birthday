...
const TimelinePhotos = () => {
  ...
  useEffect(() => {
    const photos = getPhotosByCategory('timeline photos');
    
    const memories = photos.map((url, i) => ({
      date: `Chapter ${i + 1}`,
      title: i === 0 ? 'The Very Beginning' : 'Building Our Story',
  ...
  useEffect(() => {
    ...
    const cards = gsap.utils.toArray('.tp-card');
  ...
  return (
    <section ref={sectionRef} className="py-32 px-4 relative flex flex-col items-center overflow-hidden bg-slate-950">
      <h2 className="text-5xl md:text-7xl font-cursive text-center mb-24 text-rose-300 text-glow">How It Started</h2>
      
      <div className="relative w-full max-w-5xl mx-auto">
        <div className="absolute left-1/2 transform -translate-x-1/2 h-full w-1 bg-gradient-to-b from-transparent via-rose-300/60 to-rose-500/10 rounded-full shadow-[0_0_15px_rgba(255,182,193,0.5)]" />
        
        {timelinePhotos.map((memory, index) => (
          <div key={index} className={`tp-card relative flex w-full my-24 ${index % 2 === 0 ? 'justify-start' : 'justify-end'}`}>
            ...
            <div className={`w-5/12 glass p-6 md:p-8 rounded-3xl hover:scale-[1.02] transition-all duration-500 shadow-[0_0_30px_rgba(255,182,193,0.1)] ${index % 2 === 0 ? 'mr-auto text-right' : 'ml-auto text-left'}`}>
